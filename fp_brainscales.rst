...
These dynamics can be configured and tuned through local :term:`SRAM` exposed to the outside via individual memory controllers.
...
Spikes can be directly injected into the :term:`ASIC` from the "outside", e.g. a host computer or -- in a multi-chip system -- other :term:`ASIC` or potentially a sensor circuit.
In these cases, the events are relayed by an :term:`FPGA` acting as a real-time communication interface.
...
In addition, background spike sources are directly provided on the :term:`ASIC` itself [#schemmel2020accelerated]_.
...
These, most importantly, encompass an enable signal (``dacen``) of typically 4 ns to gate the current emitted by the synapse :term:`DACs<DAC>`.
...
On a full-size BrainScaleS-2 :term:`ASIC`, both synapse arrays respectively host 256 rows and thus feature 128 synapse driver instances.
...
Their primary function is to convert afferent events into analog current pulses and for that purpose they each employ a 6 bit :term:`DAC`.
...
For that purpose, they each store a 6 bit source address in their local :term:`SRAM` alongside their weight.
...
Being stored in local :term:`SRAM`, these labels can be dynamically changed to alter the emulated network's topology during an experiment.
...
These sensors represent the accumulated correlation traces on two capacitors, which can be read out via dedicated parallel :term:`ADCs<ADC>`.
Their states can be specifically reset to start a new measurement interval.
4 bit of local :term:`SRAM` allow to coarsely equalize production-induced fixed-pattern deviations across the synapse array.

Neurons
^^^^^^^

A full-size BrainScaleS-2 :term:`ASIC` houses 512 neuron circuits, distributed over two horizontal rows of 256 neurons each.
They implement the AdEx model but can be reduced to the LIF model or even non-time-continuous integrators, e.g. for the realization of :term:`ANNs<ANN>`.
...
The neuron circuit and its backend can be configured via a total of 64 bit of local :term:`SRAM` and the analog components can be tuned through 8 reference voltages and 16 current parameters per neuron.
...
Each parameter can be freely programmed as a digital 10 bit value stored in full-custom :term:`SRAM`.

This highly parallel :term:`DAC` is realized as an array of capacitive storage cells [#hock2014modern]_.
Each of the latter consists of a small capacitor and 10 bit of local :term:`SRAM`.
Voltage cells are straightforwardly realized by tapping the potential across that capacitor to provide output potentials in the range from approximately 0.0 to 1.6 V, while current cells require an additional :term:`MOSFET` to translate this potential into a current ranging from approximately 0.0 to 1.0 µA.
The cells are programmed based on a periodically cycling voltage ramp accompanied by a digital 10 bit counter.
The memory array basically functions as a content-addressable memory: whenever the counter value matches the content of an :term:`SRAM` cell, the associated capacitor is, simply put, connected to the voltage ramp and in that process inherits the potential corresponding to that specific point of the ramp cycle.
...
Peripheral circuits such as the capacitive analog parameter storage, similarly, provide access to some of their internal mechanisms to facilitate the commissioning of the :term:`ASIC`.
...
One may generally differentiate between unidirectional readouts, many of which already buffered, and bidirectional :term:`IOs<IO>` offering a low-impedance path to the internal circuits.
...
An on-chip :term:`ADC`, furthermore, allows to digitize these two channels directly on the :term:`ASIC`.
...
The :term:`ADC` itself implements an successive-approximation register architecture and -- in the current setup -- permits sampling rates of 30 MSample/s.
The samples are streamed out via the high-speed communication links in the form of event packages [#karasenko2020neumann]_.

Two additional multiplexers, directly connected to two analog :term:`IO` pads of the :term:`ASIC` [#kiene2017mixed]_, allow to connect external devices and measurement equipment.
They allow to select not only the two buffered channels mentioned above but many other internal nodes either exceeding the 1.2 V supply range (and thus the :term:`ADC`'s input range) or requiring unbuffered, bidirectional access.
The two pads can be attached to :term:`DACs<DAC>` located on the experiment setup :term:`PCB` or external laboratory equipment [#schreiber2021accelerated]_.
...
BrainScaleS-2, hence, features massively parallel :term:`ADCs<ADC>` to still be able to incorporate these states into plasticity calculations.

The two :term:`ADCs<ADC>` [#schreiber2021accelerated]_ -- one per vertical half of the :term:`ASIC` -- each feature 512 channels, two per column of synapses to cover both the causal and anti-causal branches of the correlation sensors.
The :term:`ADCs<ADC>` employs a ramp-compare architecture to digitize the parallel channels at a resolution of 8 bit and a maximum sampling rate of 1.85 MSample/s per channel [#schreiber2021accelerated]_.
...
BrainScaleS-2 augments the accelerated dynamics of the analog neuromorphic core with custom embedded processors [#friedmann2016demonstrating]_ [#friedmann2013new]_, one per vertical half of the :term:`ASIC`.
These :term:`PPUs<PPU>` implement the Power instruction set architecture and operate on 32 bit integers.
They are clocked at a configurable frequency defaulting to 250 MHz.
Each processor instance has access to 16 kiB of main :term:`SRAM` collocating instructions as well as data.
They can, furthermore, transparently access a larger, shared memory region provided by the :term:`FPGA` and made available via the high-speed communication links.
As the :term:`PPU` can act as masters to the on-chip configuration bus, they have full read and write access to all on-chip components and can thus read out and operate on, e.g., the neuronal firing rates and, in addition, dynamically reparameterize all of the :term:`ASIC['s]<ASIC>` subsystems.

The general purpose parts are accompanied by custom :term:`SIMD` vector extensions [#friedmann2016demonstrating]_ [#friedmann2013new]_.
...
Responses to read requests are buffered and tracked on the :term:`FPGA`.
...
The software stack also boasts full compiler support for the embedded :term:`PPU`, allowing to program these custom processors in standard C++ code.
...
A large fraction of the system's functionality is, however, also available through two high-level :term:`APIs<API>`:
...
A PyTorch-compatible interface, on the other hand, allows the emulation of arbitrary :term:`ANNs<ANN>` and convolutional networks [#paszke2019pytorch]_ [#spilger2020hxtorch]_.
